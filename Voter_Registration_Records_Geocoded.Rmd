...
df %>%
  ...
  select(county_emsid, surname, Latitude, Longitude) %>%
  write.csv(file = "voter_geometry.csv", row.names = FALSE)
```
...
Note: the way NGP VAN's VoteBuilder platform does is they provide users the ability to select the probability of each race. For example, their column is called "Likely Ethnicity" and when you create filters for your voter list, you can select Asian High, Asian Medium and Asian Low (based on probability).
```{r}
...
```
For some reason, the wru function did run properly the third time around so I reloaded the interim csv I saved last round back in.
```{r}
df_full = read_csv("voter_registration_full.csv")
#View(df_full)

#fixing the parsing issues due to the "geometry" column
df_sf_block = df_full

df_sf_block[,50:72] = df_sf_block[,51:73]
df_sf_block = df_sf_block[, 1:72]
...
add age_group to facilitate 2022 and 2020 turnout % calculate in Voting_History worksheet
```{r}
df_sf_block = df_sf_block %>%
  #count(age, name="UniqueNames")
  mutate(age_group = case_when(
    ...
    age >= 50 & age <= 64 ~ "50-64",
    age >= 65 ~ "65+"),
    age_group_gender = paste(age_group, " ", gender)) #%>%
  #select(age, age_group, gender, age_group_gender)
```
...
df_sf_block %>%
  ...
  summarise(cnt = n()) %>%
  mutate(proportion = cnt/sum(cnt)*100) %>%
  ggplot(aes(y=proportion, x=age_group)) +
  #ggplot(aes(y=proportion, x=fct_reorder(age_group, proportion, .desc=FALSE))) +
  geom_bar(position="dodge", stat="identity", fill="brown") +
  geom_text(aes(label = paste0(round(proportion,2), "%")), vjust = -0.5, color = "black", size = 3) +
  labs(title = "Age composition among registered voters in Queens New York", x = "age_group") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(color = "black"),
...
ggsave("viz/age_composition.png")
```
...
df_sf_block %>%
  #as.data.frame() %>%
  select(county_emsid, gender, race_est, age_group_gender) %>%
...
Merge voter registration with voting history records
```{r}
df_sf_block = df_sf_block %>%
...
df_sf_block %>%
  write.csv(file = "voter_registration_full_fixed.csv", row.names = FALSE)
...
cols_required = df_sf_block %>%
  ...
  select(county_emsid, surname, first, middle, assembly_district, address, apartment_number,
         gender, age, party_full, telephone_optional,email_address)

# add assembly_district to top 100 buildings
address_assembly = df_sf_block %>%
  as.data.frame() %>%
  select(address, assembly_district) %>%
  distinct() #%>%
  #get_dupes(address) # there is one to one relationship between the addrress and assembly_district
...
df_output = df_sf_block %>%
  ...
  left_join(address_assembly, by="address")
...
campaign ask: flagging triple prime voters (defined by the DEM voters who have voted three times in the past three years' DEM primaries)
```{r}
df_prime = df_sf_block %>%
  #as.data.frame() %>%
  filter(party_full == "Democratic") %>%
  group_by(address) %>%
  summarise(cnt_dem = n()) %>%
...
# Split the data frame based on "address"
district_list <- split(df_prime, df_prime$assembly_district)
district_list
# create the output folder
destination_path = "Democrat_prime_voters"
dir.create(destination_path, showWarnings = FALSE)
#destination_path

# Save each individual data frame as a CSV file
lapply(names(district_list), function(district) {
  #building_rank = unique(building_list[[building]]$rank_)[1]
  file_name = paste0(destination_path, "/", "assembly_district ", district, ".csv")
  # sort the data in each building by floor number then apartment number
  # building_list[[building]] = building_list[[building]][order(building_list[[building]]$floor_num,
  #                                                             building_list[[building]]$apartment_number), ]
  # move the floor_num column before the apartment_number column to the beginning
  # building_list[[building]] = building_list[[building]][, c("floor_num", "apartment_number", setdiff(names(building_list[[building]]), c("floor_num", "apartment_number")))]
  write.csv(district_list[[district]], file = file_name, row.names = FALSE)
})
```

campaign ask: flagging voters above 60 years of age and group the list by assembly district
```{r}
df_60above = df_sf_block %>%
  #as.data.frame() %>%
  filter(party_full == "Democratic" & age >= 60) %>%
  group_by(address) %>%
  summarise(cnt_dem = n()) %>%
  arrange(desc(cnt_dem)) %>%
  mutate(rank_ = rank(-cnt_dem, ties.method = "min")) %>%
...
# Split the data frame based on "address"
district_list <- split(df_60above, df_60above$assembly_district)
district_list
# create the output folder
destination_path = "Democrat_voters_above60"
dir.create(destination_path, showWarnings = FALSE)
#destination_path
...
campaign ask: send a list of the top 100 buildings with ranking, address, and the Assembly district numbers
```{r}
# add assembly_district to top 100 buildings
address_assembly = df_sf_block %>%
  as.data.frame() %>%
...
address_assembly %>%
  count(assembly_district, name="UniqueNames") #18 assembly districts in Queens
...
df_sf_block %>%
  ...
  group_by(address) %>%
  summarise(cnt_dem = n()) %>%
  top_n(100, wt = cnt_dem) %>%
  arrange(desc(cnt_dem)) %>%
  mutate(rank_ = rank(-cnt_dem, ties.method = "min")) %>% # Gives the smallest rank to tied values
  left_join(address_assembly, by="address") %>%
  write.csv(file = "top100building with assembly district number.csv", row.names = FALSE)

```


how to reach your vote goal? vote goal/winning number is about 200,000 votes. That translates to top 3,785 buildings/houses in Queens county
```{r}
View(df_sf_block)
View(df_voter_history_pri21to23)

# select fields that are being asked to be included in the voters list
df_sf_block_mini = df_sf_block %>%
  #as.data.frame() %>%
  filter(party_full == "Democratic") %>%
  select(county_emsid, surname, first, middle, gender, age, race_est, is_Chinese,
         telephone_optional, email_address, address, apartment_number,
         assembly_district, election_district) %>%
  left_join(df_voter_history_pri21to23, by="county_emsid") %>%
  mutate_at(vars(c("triple_prime", "double_prime")), ~ifelse(is.na(.), 'N', .))

df_sf_block_mini

# 42,161 triple prime voters
df_sf_block_mini %>%
  filter(triple_prime == "Y") %>%
  group_by(race_est) %>%
  summarise(cnt = n()) %>%
  mutate(proportion = (cnt/sum(cnt))*100) %>%
  ggplot(aes(y=proportion, x=fct_reorder(race_est, proportion, .desc=TRUE))) +
  geom_bar(position="dodge", stat="identity", fill="skyblue") +
  geom_text(aes(label = paste0(round(proportion,1), "%")), vjust = -0.5, color = "black", size = 3) +
  labs(title = "Race composition among triple prime voters in Queens New York", x = "race") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(color = "black"),
...
ggsave("viz/race_composition_triple_prime.png")
...
df_sf_block_mini %>%
  ...
  coord_flip() +
  theme_classic() +
  labs(title = "Triple prime voters age box plot by race", x = "race") +
  theme(legend.position="none")

ggsave("viz/triple prime age box plot by race.png")

```

vote goal calculator. We can create some tiers here
```{r}
...
# 60,650 double prime voters
df_sf_block_mini %>%
  filter(double_prime == "Y")

# 50,517 Chinese voters that are not triple nor double prime voters
df_sf_block_mini %>%
  filter(double_prime == "N" & triple_prime == "N" & is_Chinese == TRUE)

# 164,516 Asian voters that are not triple nor double prime voters
df_sf_block_mini %>%
  filter(double_prime == "N" & triple_prime == "N" & race_est == "asian")

# 271,167 elder voters that are not triple nor double prime voters
df_sf_block_mini %>%
  filter(double_prime == "N" & triple_prime == "N" & age >= 55)
...
Create bar chart to show race composition by number for assembly district in Queens
We will focus on Democratic party
```{r}
df_sf_block %>%
  #as.data.frame() %>%
  filter(party_full== "Democratic") %>%
  group_by(assembly_district, race_est) %>%
  summarise(cnt = n()) %>%
  mutate(proportion = cnt/sum(cnt)*100) %>%
  ggplot(aes(fill=race_est, y=cnt, x=as.factor(assembly_district))) +
  geom_bar(position="stack", stat="identity") +
  coord_flip() +
  #geom_text(aes(label = cnt, position = "stack", vjust = 1.0, color = "black", size = 3)) +
  labs(title = "Race composition by assembly district in Queens New York", x = "assembly district", fill = "race") +
  theme_classic()

ggsave("viz/party composition by assembly district.png")
```

Create bar chart to show race omposition by % for assembly district in Queens
We will focus on Democratic party
```{r}
df_sf_block %>%
  #as.data.frame() %>%
  ...
  summarise(cnt = n()) %>%
  mutate(proportion = cnt/sum(cnt)*100) %>%
  ggplot(aes(fill=race_est, y=proportion, x=as.factor(assembly_district))) +
  geom_bar(position="stack", stat="identity") +
  #coord_flip() +
  #geom_text(aes(label = cnt, position = "stack", vjust = 1.0, color = "black", size = 3)) +
  geom_text(aes(label = paste0(round(proportion,1), "%")), position = "stack", vjust = 1.0, color = "black", size = 1) +
  labs(title = "Race composition by assembly district in Queens New York", x = "assembly district", fill = "race") +
  theme_classic()
  
#ggsave("viz/party composition by race.png")
```
It doesn't seem that census tract and assembly district have one to one relationship, neither does census block
```{r}
df_sf_block %>%
  select(TRACTCE10, assembly_district) %>%
  distinct() %>%
  get_dupes(TRACTCE10)

df_sf_block %>%
  select(BLOCKCE10, assembly_district) %>%
  distinct() %>%
  get_dupes(BLOCKCE10)


df_sf_block %>%
  select(assembly_district, Longitude, Latitude) %>%
  distinct() %>%
  get_dupes(assembly_district)
...
you have close to 20% of registered voters who have never voted
```{r}
df_sf_block %>%
  as.data.frame() %>%
  group_by(voted_) %>%
...
st_join(df_sf_block_copy, ny_income, join = st_within)
```
